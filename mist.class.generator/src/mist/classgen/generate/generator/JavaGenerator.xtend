...
class JavaGenerator{
    	...
    	.ui-helper-hidden-accessible {
    	  ...
    	  width: 1px;
   		    	...
   		    	«ENDFOR»   		'''    }        def generateTableStyle(Entity vClass){    	'''    	    			«IF vClass.overviewsettings != null && vClass.overviewsettings.tableoverview!= null && vClass.overviewsettings.tableoverview.width>0»    			#«vClass.name»Table{    				width: «vClass.overviewsettings.tableoverview.width»px;    			}    			«ENDIF»    	'''    }        def generateClassOverview(Entity validationClass){    	'''    		<div id="overview«validationClass.name»" class="panel classOverview">    			<div class="row">    			<div class="col-1 sideBar">    			<button type="button" class="btn delete-mode" >
    				...
    				</li>    			«ENDIF»    				«FOR attr : validationClass.attributes»    					«IF attr.attributetype.dataType instanceof NonPrimitiveDataType»    					«var refClass = attr.attributetype.dataType as NonPrimitiveDataType»    					    					 <li class="nav-item">
    ...
    def generateAttributeRules(AttributeRule rule, String attributeName){
    	...
    	'''
    }                def generateAttributeRulesMessage(AttributeRule rule, String attributeName){		'''			«IF rule instanceof Required»			«IF rule.errorMessage!= null && rule.errorMessage.message!=""»			required: "«rule.errorMessage.message»"			«ENDIF»			«ELSEIF rule instanceof StringPattern»			«IF rule.errorMessage!= null && rule.errorMessage.message!=""»
			pattern: "«rule.errorMessage.message»"
			«ENDIF»			«ELSEIF rule instanceof Length»
			«IF rule.errorMessage!= null && rule.errorMessage.message!=""»
			length: "«rule.errorMessage.message»"
			«ENDIF»			«ELSEIF rule instanceof MaxLength»
			«IF rule.errorMessage!= null && rule.errorMessage.message!=""»
			maxlength: "«rule.errorMessage.message»"
			«ENDIF»			«ELSEIF rule instanceof MinLength»
			«IF rule.errorMessage!= null && rule.errorMessage.message!=""»
			minlength: "«rule.errorMessage.message»"
			«ENDIF»			«ELSEIF rule instanceof MinNumber»
			«IF rule.errorMessage!= null && rule.errorMessage.message!=""»
			min: "«rule.errorMessage.message»"
			«ENDIF»			«ELSEIF rule instanceof MaxNumber»
			«IF rule.errorMessage!= null && rule.errorMessage.message!=""»
			max: "«rule.errorMessage.message»"
			«ENDIF»			«ENDIF»		'''    }
    ...
    def generateConstraintEquals(Equals equalsRule){
    	...
    	'''
    		...
    		$(tabId).hide();    		var $items = $(".tabPanel-li").find("a");    		if($items.length>0){    			var element = $items[0];    			var el = $items.get(0);    			var $element =$(el);    			$(el).attr("class", "nav-link active");    			var $element1 = $(el);    			var panelSelector = $(el).attr("href");    			$(".panel").hide();    			$(panelSelector).show();    			event.stopImmediatePropagation();    		}else{    		}
    		 ...
    		 $panelToShow.show();
    	 });    	'''    }        def generateGlobalVariableReferencedClasses(Entity vClass){        	val referencedClasses = new ArrayList();    	var referencingAttributes = new ArrayList();
    	  for(attribute : vClass.attributes){
    	    if(attribute.attributetype.dataType instanceof NonPrimitiveDataType)
    	    {    	    	var dataType = attribute.attributetype.dataType as NonPrimitiveDataType;    	    	var refClass = dataType.type;
